...
def build_topside() -> bool:
    ...
    logging.info(
        f"[Topside] Setting working dir to {currentDirectory}/lib/navigator-lib")
    ...
    cmake_configure = subprocess.run(
        ["cmake", "-B", "build", "-G", "Ninja"], capture_output=True)
    ...
    cmake_build = subprocess.run(
        ["cmake", "--build", "build", "--target", "topside"], capture_output=True)
...
def main():
    ...
    logging.info('Successfully built topside!')
    ...
    logging.info('Successfully built rov!')
